...
//
//  Created by Mac-Qke on 2019/7/4.
...
- (IBAction)showInfoClick:(UIButton *)sender {
...
//        NSLog(@"count: %d",count);
    ...
    for (int i = 0; i < count; i++) {
        ...
        NSLog(@"%s: %s",memberType,memberType);
    }
    ...
    for (LSApplicationProxy_class in appList) {
        ...
        NSObject *localizedShortName = [LSApplicationProxy_class performSelector:@selector(locallizedShortName)];
